...
- PAGE-инструкция не содержит «Пропустить» и «Закрыть»: доступны «Назад / Далее / Сначала», мышь и клавиатура.
- `Escape` не отключает обязательный PAGE-tour; Tab может перейти из немодальной карточки к рабочему интерфейсу.
...
- PAGE-tour автоматически работает при каждом переходе и циклически возвращается к первому шагу; ROLE/THESIS после завершения возвращают пользователя к PAGE-инструкции.
